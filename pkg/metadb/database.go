...
func (db *MetaDB) PutFBIDForIGUser(ctx context.Context, igid string, fbid int64) error {
	...
	_, err := db.Exec(ctx, "INSERT INTO meta_instagram_user_id (igid, fbid) VALUES ($1, $2) ON CONFLICT (igid) DO UPDATE SET fbid = $2", igid, fbid)
	return err
}

func (db *MetaDB) PutFBIDForIGThread(ctx context.Context, igid string, fbid int64) error {
	_, err := db.Exec(ctx, "INSERT INTO meta_instagram_thread_id (igid, fbid) VALUES ($1, $2) ON CONFLICT (igid) DO UPDATE SET fbid = $2", igid, fbid)
	return err
}
