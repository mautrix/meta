...
func (m *MetaClient) decryptPush(push json.RawMessage) ([]byte, error) {
	var wpd webPushData
	...
	keyInfo := buildLegacyInfo("Content-Encoding: aesgcm\x00", keys.P256DH, dh.Bytes())
	aesKey := hmacSHA256(prk, append(keyInfo, 0x01))[:16]

	nonceInfo := buildLegacyInfo("Content-Encoding: nonce\x00", keys.P256DH, dh.Bytes())
...
func buildLegacyInfo(label string, clientPub, serverPub []byte) []byte {
	...
	_ = binary.Write(&buf, binary.BigEndian, uint16(len(clientPub)))
	buf.Write(clientPub)
	_ = binary.Write(&buf, binary.BigEndian, uint16(len(serverPub)))
	buf.Write(serverPub)
	return buf.Bytes()
}
