...
func (i *Interpreter) Evaluate(ctx context.Context, form *BloksScriptNode) (*BloksScriptLiteral, error) {
	...
	case "bk.action.io.CurrentTimeMillis":
		// Android's System.currentTimeMillis() returns a signed 64-bit count of
		// milliseconds since the Unix epoch. Keep that representation so Bloks
		// timestamp arithmetic doesn't lose precision.
