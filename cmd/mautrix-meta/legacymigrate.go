...
func migrateLegacyConfig(helper up.Helper) {
	...
	bridgeconfig.CopyToOtherLocation(helper, up.Str|up.Null, []string{"meta", "proxy"}, []string{"network", "proxy"})
	bridgeconfig.CopyToOtherLocation(helper, up.Str|up.Null, []string{"meta", "get_proxy_from"}, []string{"network", "get_proxy_from"})
